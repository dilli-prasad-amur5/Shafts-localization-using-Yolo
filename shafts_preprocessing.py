...
def read_annotations(img_dir):
    ...
    for ann_file_name in sorted(os.listdir(img_dir)):  # ann_file_name stores all images and label files
        ext = os.path.splitext( ann_file_name )[1]     # os.path.splitext() function returns a tuple containing the file name and extension of the input path
                                                       # we are only interested in the extension, we access the second element of the tuple(i.e, .txt, .bmp)
...
class YoloBatchGenerator(Sequence):
    ...
    def __len__(self): # returns the number of batches in the dataset.
