...
class ShaftsYOLO(object):
    ...
    def custom_loss(self, y_true, y_pred):
        mask_shape = tf.shape(y_true)[:4] # mask_shape is then (batch_size, gridsize_x, gridsize_y, 1) and 1 represent no of bounding
        ...
        # masks are used for fast loss computation and Intialize all masks with 0
        # By initializing the mask tensors to zeros, we create a blank slate where no objects are initially present,
        # and then use them to mark the positions where objects are present in the training data.
        ...
        # adding cell_grid to last two dimensions of t_pred
        ...
        # The ... is used for indexing to mean that all dimensions before the last one should be included.
    ...
    def get_f1(self, y_true, y_pred): #shapes (batch, 4)
        ...
        # ground_positives represents the total number of ground truth objects in the batch  summing up the confidence scores of ground truth objects 
        # in all cells of the batch.
        ground_positives = tf.reduce_sum(y_true[..., 4]) + 1e-6  # Adding a small constant 1e-6 is for numerical stability, in case ground_positives is zero.
        
        #if true conf and pred_conf>0.5 
        # true_positives represents the number of true positive detections made by the model
        true_positives = tf.reduce_sum(tf.cast(true_conf > 0.5, dtype=tf.float32) * tf.cast(pred_conf > 0.5, dtype=tf.float32)) + 1e-6
        
        #total number of predicted objects in the batch, obtained by summing up the confidence scores predicted by the model for all cells of the batch.
        pred_positives = tf.reduce_sum(y_pred[..., 4]) + 1e-6  
       
    
        precision = true_positives / (pred_positives + 1e-6)  # Precision measures how accurately the model detects the objects,
        recall = true_positives / (ground_positives + 1e-6)   # recall measures how many of the actual objects are detected by the model.
        ...
        # F1 score multiplying it by the number of ground truth positives and dividing by the total number of ground truth positives in the dataset.
